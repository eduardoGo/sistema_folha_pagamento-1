...
public class DadosEmpregados {
	...
	public void newEmployer() {
		...
		Empregado newEmployee = new Empregado(name, adress);
		
		newEmployee.PutNewTypeEmployee();
		...
		do {
			...
			switch(option) {
				...
				case 4:
					...
					break;
				case 5:
					Sindicato.enterOrExit(currentEmployee);
					break;
				case 6:
					Sindicato.editSyndicateFee(currentEmployee);
					break;
				case 7:
	...
	public void addSalesResult() {
		...
		currentEmployee.getSales().add(new Sale());
	}
	...
	private boolean mustBePaid(Empregado employee,Date todayDate) {
		...
		int day,total;
		int daysWorkeds=0;
		int[] daysPerMonth = todayDate.getDayspermonth();
	...
	private void pay(Empregado employee) {
		...
		float deductios = deductions(employee);
		float salesValue = 0;
		System.out.printf("Gross salary: %.2f%n",employee.getGrossSalary());
		if(employee.getTypeEmployee() == TypeEmployee.COMISSIONED) {
			salesValue = totalSalesValue(employee);
			System.out.printf("Sales made: %.2f%n", salesValue);
			
		}
		else if(employee.getTypeEmployee() == TypeEmployee.PAYED_BY_HOUR) {
			System.out.printf("Extra Hours: %d", employee.getExtraHours());
		}
		
		System.out.printf("Net salary: %.2f%n", employee.getGrossSalary() + employee.getExtraHours()*5 
				+ salesValue*0.15 - deductios);
		
		employee.setExtraHours(0);
	...
	private float totalSalesValue(Empregado employee) {
		...
		float total=0;
		for(Sale auxSale : employee.getSales())
			total += auxSale.getValue();
		
		return total;
	}
	
	private float deductions(Empregado employee) {
